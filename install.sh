...
if $IS_UPDATE; then
  ...
  for k in "${OPENAI_API_KEY:-}" "${NVIDIA_API_KEY:-}" "${GOOGLE_API_KEY:-}" \
            "${DEEPSEEK_API_KEY:-}" "${GROQ_API_KEY:-}" "${ANTHROPIC_API_KEY:-}" \
            "${SILICONFLOW_API_KEY:-}" "${MOONSHOT_API_KEY:-}" \
            "${ZHIPU_API_KEY:-}" "${DASHSCOPE_API_KEY:-}"; do
    [ -n "$k" ] && HAS_AI_KEY=true
  done
...
DEEPSEEK_API_KEY=
GROQ_API_KEY=
# Chinese providers (free tiers available — see siliconflow.cn, platform.moonshot.cn)
SILICONFLOW_API_KEY=
MOONSHOT_API_KEY=
ZHIPU_API_KEY=
DASHSCOPE_API_KEY=
